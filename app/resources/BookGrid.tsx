...
const BookSection = ({ title, books }: { title: string; books: Book[] }) => {
  ...
  const displayedBooks = showAll ? books : books.slice(index, index + ITEMS);

  return (
    <div className="relative w-full mx-auto px-4 md:px-0">
      {/* Header - Simplified without negative margins */}
      ...
      {/* Navigation Arrows - Only show when not in "View All" mode */}
      {!showAll && books.length > ITEMS && (
        <>
          {/* Prev Arrow - Adjusted positioning */}
          <Image
            src="/leftArrow.png"
            alt="Previous"
          ...
          {/* Next Arrow - Adjusted positioning */}
          <Image
            src="/rightArrow.png"
            alt="Next"
      ...
      <div className={`
        max-w-5xl mx-auto
        ${showAll 
          ? "grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 gap-4 md:gap-6" 
          : "flex gap-6 overflow-x-auto md:overflow-visible md:grid md:grid-cols-4 lg:grid-cols-4 pb-4 md:pb-0"
        }
      `}>
          ...
          >
            <div className="space-y-4">
              ...
              <div className="text-left space-y-1 md:space-y-2">
                ...
                <p className="font-clash-grotesk text-xs lg:text-base text-white/70 line-clamp-2">
                  {book.author}
                </p>
              </div>
            </div>
